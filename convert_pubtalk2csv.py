...
FILE1 = 'test.txt'
SIMPLE_MARKDOWN = 'simple-markdown.md'
PROMPT_PRE_1 = 'What is this all about?'

PROMPT_DIR = 'prompts/'
PT_DIR = 'pt_word_documents/'
...
def prompt_on_expanded_text(prompt_text, text_to_process):
    ...
    response = ollama.generate(model='gemma3', prompt=my_prompt)
...
def convert_pt_word_doc_to_csv_for_wix_import(fname):
    ...
    print(f'Full pathname to file is "{doc_abs_path}"')
    print(f'Input file (relative) is "{full_doc_name}"')
    print(f'Input file (absolute) is "{doc_abs_path}"')
    print(f'CWD is "{cwd}"')

    doc_path_to_use = doc_abs_path
    ...
    wrapperd_csv_output = prompt_on_expanded_text(extract_prompt, content_in_md_format)
    csv_output = obtain_pure_csv(wrapperd_csv_output)
    _print_content(f'DERIVED OUTPUT IS CSV', csv_output)
